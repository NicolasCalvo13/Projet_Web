...
// Route pour supprimer une offre depuis la page Wishlist
...
$router->get('reviews_company', function () use ($twig) {
    // ⚠️ On change le contrôleur et la méthode ici
    $controller = new UserController($twig);
    return $controller->company_reviews();
});

// Déconnexion et redirection vers le login entreprise
$router->get('switch_to_company', function () use ($twig) {
    $controller = new AuthController($twig);
    return $controller->switchToCompany();
});
