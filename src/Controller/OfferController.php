...
/**
     * Gère la requête AJAX pour ajouter/retirer un favori
     */
    public function toggleWishlistAjax(): void
    {
        header('Content-Type: application/json');
        ...
        // On récupère les données envoyées en JSON (fetch)
        $data = json_decode(file_get_contents('php://input'), true);
        $offerId = $data['offer_id'] ?? null;

        if ($studentId && $offerId) {
            // On appelle la fonction toggle dans le modèle
            $success = $this->model->toggleWishlist((int)$offerId, (int)$studentId);
            
            if ($success) {
                // On vérifie le nouvel état pour informer le bouton
                $isActive = $this->model->isOfferInWishlist((int)$offerId, (int)$studentId);
